...
# from models.inception1d import inception1d, inception1d_decoupled
...
def multi_triplet_loss(alpha=1., beta=0.0, gamma=0.01, margin=0.5):
    def loss(multi, y_true):
        ...
        # cls = F.binary_cross_entropy_with_logits(input=logits, target=y_true, reduction='none')
        # pos_weight_tensor = torch.tensor([ 6.4913,  4.6323,  8.5249, 28.1398,  2.7033, 10.1640*1.5,  8.8243,  6.9137, 30.2591]).cuda()
        # pos_weight_tensor = torch.tensor([ 6.4913,  4.6323,  8.5249, 28.1398,  2.7033, 10.1640,  8.8243,  6.9137, 30.2591]).cuda()

        # cls = F.binary_cross_entropy_with_logits(input=logits, target=y_true, pos_weight = pos_weight_tensor)
        cls = F.binary_cross_entropy_with_logits(input=logits, target=y_true)
        ...
        # cls =  alpha * cls + beta * rls + gamma * mls

        # return (alpha * cls + beta * rls + gamma * mls + 0.2*pt).mean()
        return alpha * cls + beta * rls + gamma * mls

        # loss = 0.25* (1 - pt) ** 2 * cls
...
#         elif (self.name.startswith("decoupled_fastai_fcn_wang")):  # note: order important for string capture
#             model = fcn_wang_decoupled(num_classes=num_classes, input_channels=self.input_channels, ps_head=self.ps_head,
#                              lin_ftrs_head=self.lin_ftrs_head, div_lin_ftrs_head=self.div_lin_ftrs_head, if_train=if_train)
