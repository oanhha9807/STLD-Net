...
ss = StandardScaler()
import pickle
class ECGDataset_image(Dataset):
    def __init__(self, phase, data_dir, label_csv, folds, leads):
        super(ECGDataset_image, self).__init__()
        self.phase = phase
    ...
    def __getitem__(self, index: int):
        ...
        #
        # ss.fit(np.vstack(ecg_data).flatten()[:, np.newaxis].astype(float))
        # ecg_data = apply_standardizer(ecg_data, ss)
        # scaler_path = os.path.join('/home/oem/oanh/DL_ECG_Classification/Decoupling/CPSC2018/scaled_data/', f'{patient_id}.npy')
        ...
        path_X = '/data/oanh/Bio/ECG_CPSC2018/images_128/'
        ...
        return torch.from_numpy(result.transpose()).float(), torch.from_numpy(image).float(), torch.from_numpy(labels).float()
