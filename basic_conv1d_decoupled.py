...
class DivOutLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # cat_out = torch.tensor(cat_out)
        cat_out = torch.unsqueeze(cat_out, dim=-1)
        x_em_deal, cat_out = x_em_deal.cuda(), cat_out.cuda()
        # print(x_em_deal.shape,cat_out.shape)
        out = self.aggre(torch.cat((x_em_deal, cat_out), dim=-1))
        # if self.if_train == True:
        #     return [out, feats]
        if self.if_train:
            return out, feats, x_em_deal.squeeze(-1), cat_out.squeeze(-1)
        else:
            return out
